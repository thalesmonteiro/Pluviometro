...
; * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
;FUNÇÃO QUE AGUARDA O BARRAMENTO FICAR LIVRE           
AGUARDA_I2C  
	BANK1
	BTFSC	SSPSTAT, R_W
	GOTO	$-1
...
; * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
;       REALIZA A ESCRITA NA EEPROM EXTERNA ATRAVES DO I2C        *
;   RECEBE O ENDERECO NAS VARIAVEIS ENDERECO_HIGH E ENDERECO_LOW  *
; REALIZA A ESCRITA DO CONTEUDO DA VARIAVEL DADO_H NA POSICAO DE  *
; MEMORIA COMPOSTA POR (ENDERECO_HIGH E ENDERECO_LOW)E O CONTEUDO *
; DE DADO_L NA POSICAO DE MEMORIA SEGUINTE.                       *
; * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * *
...
ESTOURO	
	...
	;ESPERA VALOR A SER DIVIDIDO NO WORK
DIVIDE_POR_10
	;AUX = DEZENA
	;AUX2= UNDIADE
...
ENDERECO_HORA
    ...
    ;ESSA ROTINA NECESSITA DO VALOR DO DIA DO MES E DO ANO
...
SOMATORIO1    
    ...
    CALL    LER_ANO
    CALL    LER_MES
    CALL    ENDERECO_DIA ;RETORNA EM ENDERECO HIGH E ENDERECO LOW
    CALL    ESCRITA_I2C_EEPROM
    
    RETURN
   
    ;IMPLEMENTAR GRAVA EM MES
    ;IMPLEMENTAR GRAVA EM ANO
;REALIZA A ESCOLHA DE QUAL DIA SE DESEJA CONSULTAR O INDICE DE CHUVA
...
CHECA_BOTAO_RB5
    ...
    GOTO    SELECT_MES
    ;EXIBIÇÃO DAS <> PARA SELEÇÃO DO MES
...
ESPERA_ESCOLHA
    ...
    GOTO    OPCAO_MES
    BTFSC   PORTB,RB5
    GOTO    PRIMEIRO_MENU  ;ESCOLHA DO DIA QUE SE DESEJA CONSULTAR A QUANTIDADE DE CHUVA
    GOTO    ESPERA_ESCOLHA
...
PRIMEIRO_MENU
    ;RETORNA O DIA SELECIONADO EM DIA, MES, ANO
    CALL    SELECAO_DIA
    ;REALIZA LEITURA DA QUANTIDADE DE CHUVA NESTE DIA
    ...
    ;CONSULTA A QUANTIDADE DE CHUVA
    ;ESPERA QUE O DIA MES E ANO ESTEJAM NAS VARIAVEIS COM MESMO NOME
    ;RETORNA O ENDERECO EM ENDERECO_HIGH E ENDERECO_LOW
    CALL    ENDERECO_DIA  ;DIA *2 + 166 + 62(MES - 1) +732(ANO -2019)
    CALL    LEITURA_I2C_EEPROM ; RETORNA EM BUFFER_L E BUFFER_H
...
VOLTAR_BOTAO
    BTFSC   PORTB,RB4  ;VERIFICA SE O BOTAO VOLTAR FOI PRECIONADO
    GOTO    EXIBE_MENU_OPCOES
    BTFSC   PORTB,RB5
    GOTO    SAI_INT
    GOTO    VOLTAR_BOTAO
    
SEGUNDO_MENU   ;VER QUANTIDADE DE CHUVA NO MES
    PAGESEL LIMPA_DISPLAY
    CALL    LIMPA_DISPLAY
    ...
    BCF	    RS
    MOVLW   0XC8          ; DEFINE O CURSOR PARA LINHA 0
    CALL    ESCREVE
    BSF	    RS	
...
CHECA_OK
    BTFSS   PORTB,RB5
    GOTO    CHECA_MAIS
    PAGESEL VISUALIZACAO_MES
    GOTO    VISUALIZACAO_MES
...
LACO
    CALL    ENDERECO_DIA ;CALCULA O ENDERECO DO DIA NO MES DESEJADO E RETORNA EM ENDERECO_HIGH ENDERECO_LOW
    INCF    MES,F
...
DEFINE_QUANT_DIA    ;DEFINE A QUANTIDADE DE DIAS QUE O MES LIDO TEM
    ;ASSUME QUE O VALOR DO MES DESEJADO JA ESTEJA NA VARIAVEL MES
    ...
    GOTO    TESTA_MES
    MOVLW   .28
    MOVWF   AUX		;MES IGUAL A FEVEREIRO, PORTANTO AUX = 28
    PAGESEL $
    RETURN 

TESTA_MES    ;VERIFICA SE O MES É MENOR OU MAIOR QUE JULHO
    MOVF    MES,W
    SUBLW   .8    ;MES - 8 
    BTFSS   STATUS,C; C = 0 NEGATIVO
    GOTO    MAIOR_JULHO
    GOTO    MENOR_JULHO

MAIOR_JULHO ;SE O MES FOR MAIOR QUE JUNHO, ENTAO OS MESES PARES TEM 31 DIAS E OS IMPARES 30 DIAS
    MOVF    MES,W
    MOVWF   AUX2
    RRF	    AUX2,W
    BTFSS   STATUS,C
...
MENOR_JULHO ;SE O MES FOR MENOR QUE JUNHO, ENTAO OS MESES PARES TEM 30 DIAS E OS IMPARES 31 DIAS
    MOVF    MES,W
    MOVWF   AUX2
    RRF	    AUX2,W
    BTFSS   STATUS,C  ; C = 1 IMPAR, C = 0 PAR
    GOTO    MES_30
    GOTO    MES_31   
...
;*** OPERACAO DE SOMA COM OPERANDOS DE 32 BIT  ***
;REGA + REGB -> REGA
...
VISUALIZACAO_MES    ;EXIBIR A QUANTIDADE DE CHUVA POR MES
    ;CALCULA O ENDERECO DE UM DIA EM UM MES ESPECIFICO
    ;VERIFICA SE O MES É 30, 31, OU 28 DIAS
    PAGESEL DEFINE_QUANT_DIA
    CALL    DEFINE_QUANT_DIA    ;RETORNA EM DIA A QUANTIDADE DE DIAS DO MES
    MOVLW   .0
    MOVWF   REGA0
    MOVWF   REGA1
    MOVWF   REGA2
...
NOVAMENTE
    CALL    ENDERECO_DIA ;ESSA FUNCAO DECREMENTA O MES
    INCF    MES,F
    PAGESEL LEITURA_I2C_EEPROM
    CALL    LEITURA_I2C_EEPROM ; RETORNA EM BUFFER_L E BUFFER_H
    PAGESEL $
    MOVF    BUFFER_L,W
    MOVWF   REGB0
    ...
    PAGESEL ADD32B
    CALL    ADD32B   ; REGA -> REGA + REGB
    
    DECFSZ  DIA,F
    GOTO    NOVAMENTE
    ;SOMA FINALIZADA, RESULTADO ESTÁ EM REGA
    
    ;EXIBE O VALOR DO RESULTADO DE CHUVA NO MES
    PAGESEL LIMPA_DISPLAY
    CALL    LIMPA_DISPLAY
...
EXIBE_NA_TELA ;ESPERA O NUMERO A SER EXIBIDO NO WORK
    PAGESEL DIVIDE_POR_10
    CALL    DIVIDE_POR_10
    MOVLW   0x30
    ADDWF   AUX,W
    CALL    ESCREVE
